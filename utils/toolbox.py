...
class FeatureBuilder:
    ...
    @staticmethod
    def getFeatureArray(graph: nx.Graph, featureNum: int = FEATURE_NUM):
        def get_sorted_values(dict1: Dict):
            return [
                value for _, value in sorted(dict1.items(), key=lambda item: item[0])
            ]

        if featureNum < 3 or featureNum > 5:
            raise ValueError("特征数不能小于3或大于5")
        ...
        clusteringCoefficient = get_sorted_values(nx.clustering(graph))
        dataDict = {
            "hits": hits,
        ...
        if featureNum >= 4:
            ...
            dataDict["coreNumber"] = coreNumber

        if featureNum == 5:
            betweennessCentrality = get_sorted_values(
                nx.centrality.betweenness_centrality(graph)
            )
            dataDict["betweennessCentrality"] = betweennessCentrality
...
class GraphTool:
    ...
    def trust_rank(
        ...
        if not seeds:
            ...
            seeds = list(set(graph.nodes).intersection(set(seeds)))  # NOTE: 避免存在未知节点
    ...
    @staticmethod
    def view_edges(data: Data):
        return DataFrame(data.edge_index.T, columns=["source", "target"])
    ...
    @staticmethod
    def read_graph(
        path: Path, weighted: bool = False, delimiter: str = ","
    ) -> nx.Graph:
        if weighted:
...
if __name__ == "__main__":
    ...
    print(edgeIndex)
    print(node_mapping)
    normalizedAdjacencyMatrix = GraphTool.get_normalizedAdjacencyMatrix_from_edgeIndex(
