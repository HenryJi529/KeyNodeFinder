...
class DoubanPreprocessor:
    ...
    def handle_genre(self, genreAlias):
        genreMap = {
            "家庭": ["家庭"],
            "爱情": ["爱情", "愛情 Romance"],
            "歌舞": ["歌舞"],
    ...
    def handle_language(self, languageAlias):
        """
    ...
    def read_raw_data(self, ratio: float = None):
        """数据读取"""
        if ratio:
    ...
    @timeit
    def preprocess_person(self, person_df):
        """person数据处理"""
        # 保留特定列
        ...
        person_df["SEX"] = person_df["SEX"].apply(
            lambda x: int(np.random.binomial(n=1, p=maleProbability, size=1))
            if x != 0 and x != 1
            else x
        )
    ...
    @timeit
    def preprocess_movie(self, movie_df):
        """movie数据处理"""
        ...
        movie_df["GENRES"] = movie_df["GENRES"].apply(
            lambda x: [self.handle_genre(genreAlias) for genreAlias in x]
        )
        ...
        movie_df["LANGUAGES"] = movie_df["LANGUAGES"].apply(
            lambda x: [self.handle_language(languageAlias) for languageAlias in x]
        )
    ...
    def save_whole_processed_data(self, data: Dict):
        path = self.processed_path / Path("data.pkl")
        with open(path, "wb") as f:
            pickle.dump(data, f)

    def read_whole_processed_data(self):
        path = self.processed_path / Path("data.pkl")
    ...
    def build_featureDataFrame(
        ...
        featureDataFrame["MEDIAN_YEAR"] = featureDataFrame["MEDIAN_YEAR"].apply(
            lambda year: np.random.uniform(low=1950, high=2015, size=1)[0]
            if year < 1900.0
            else year
        )
        ...
        featureDataFrame["SEX"] = featureDataFrame["SEX"].apply(
            lambda x: int(np.random.binomial(n=1, p=maleProbability, size=1)[0])
            if x != 0 and x != 1
            else x
        )
    ...
    @timeit
    def build_featureArray(self, featureDataFrame, movieOtherInfo):
    ...
    def initialize(self):
        ...
        movie_df, person_df = self.read_raw_data()
        colored_print("处理movie.csv...")
        movie_df, movieOtherInfo = self.preprocess_movie(movie_df)
        colored_print("处理person.csv...")
        person_df, personOtherInfo = self.preprocess_person(person_df)
        ...
        for endYear in range(*self.year_range):
            ...
            featureDataFrame = self.build_featureDataFrame(
            ...
            featureArray = self.build_featureArray(featureDataFrame, movieOtherInfo)
            ...
            print("保存graph...")
            graph = self.build_graph(movie_df=movie_df[movie_df["YEAR"] < endYear])
            print("保存graph...")
        ...
        self.save_whole_processed_data(data)

        colored_print("Done!!!")


class ProteinPreprocessor:
    RAW_FILENAME = "ProteinDataset-raw.zip"

    def __init__(self, root_path: Path = REAL_DATA_PATH / "protein"):
        self.root_path = root_path
        self.raw_path = self.root_path / "raw"
        self.raw_file = self.raw_path / "HI-II-14.tsv"
    ...
    def get_raw_graph(self):
        return GraphTool.read_graph(self.raw_file, delimiter="\t")


if __name__ == "__main__":

    def test_douban_preprocessor():
        colored_print("测试豆瓣数据预处理器...")
        douban_preprocessor = DoubanPreprocessor((2000, 2002))
        data = douban_preprocessor.read_whole_processed_data()
